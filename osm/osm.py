...
class OSM:
    def __init__(self, section_name) -> None:
        ...
        # Check if Cache File exists, else generate new one
    ...
    def post(self, url, scope, cachefile, data = None, json_output = True):
    ...
    def get(self, url, scope, cachefile, json_output = True):
        cache = get_cache(cachefile)
        if cache:
            return cache
        else:
    ...
    def get_scouts(self):
        ...
        return self.get(url = url, scope = 'member', cachefile = f'{self.id}_scouts')['items']
    ...
    def create_flexi_column(self, extraid, column_name):
        ...
        column_config = self.post(url = url, scope = 'flexi', data = data, cachefile = f'{self.id}_{extraid}')['config']
    ...
    def create_flexi_record(self, flexi_record_name, type = 'maths'):
        ...
        return self.post(url = url, scope = 'flexi', data = data, cachefile = f'{self.id}_{flexi_record_name}')['id']
    ...
    def get_flexi_column_config(self, flexi_record):
        ...
        return self.get(url = url, scope = 'flexi', cachefile = f'{self.id}_{flexi_record}')['items']
    ...
    def get_badge_records_by_member(self):
        ...
        return self.get(url = url, scope = 'badge', cachefile = f'{self.id}_member_badges')
    ...
    def get_badge_record_by_identifier(self, badge_identifier):
        ...
        url = f'{OSM_BASE_URL}/ext/badges/records/?action=getBadgeRecords&term_id={self.current_term}&section={self.name}&badge_id={badge_id}&section_id={self.id}&badge_version={badge_version}'
        return self.get(url = url, scope = 'badge', cachefile = f'{self.id}_{badge_identifier}')
    ...
    def get_badge_structure_by_type(self, type_id):
        ...
        return self.get(url = url, scope = 'badge', cachefile = f'{self.id}_{type_id}')
    ...
    def get_chief_scout_badge(self):    
        ...
        badge_structure = self.get(url = url, scope = 'badge', cachefile = f'{self.id}_chief_scout')
